<template>
  <div class="article-container">
    ...
    <div class="main-wrap">
      ...
      <template v-else>
        <!-- 加载完成-文章详情 -->
        <div class="article-detail" v-if="!isLoading && !!article.art_id">
          <!-- 文章标题 -->
          ...
          <ArticleComment
            ...
            @repaly-show="comment = $event;isRepalyShow = true"
          >
          </ArticleComment>
        </div>
        <!-- /加载完成-文章详情 -->
    ...
    <!-- 底部区域 -->
    <div class="article-bottom" v-if="!isLoading">
      <van-button
        class="comment-btn"
        type="default"
        round
    ...
    <!-- 评论回复 -->
    <van-popup v-model="isRepalyShow" position="bottom" style="height: 95%">
      <RepalyComment
        @close="isRepalyShow = false" :comment="comment"
        v-if="isRepalyShow"></RepalyComment>
    </van-popup>
    <!-- /评论回复 -->
  </div>
</template>
...
<script>
import RepalyComment from './components/RepalyComment'
...
export default {
  ...
  async created () {
    try {
      ...
      if (err.response.status && err.response === 404) {
